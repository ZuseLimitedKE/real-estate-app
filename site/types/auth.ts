...
export const agencyRegistrationSchema = z
  .object({
    ...
    taxId: z.string().min(5, "Tax ID is required").max(50),
    establishedDate: z.string().refine((date) => new Date(date) <= new Date(), {
      message: "Established date cannot be in the future",
    }),
    contactPerson: z.object({
      firstName: z
        .string()
        .min(2, "Contact person first name is required")
        .max(50),
      lastName: z
        .string()
        .min(2, "Contact person last name is required")
        .max(50),
    ...
    website: z
      .url({ protocol: /^https?$/, error: "invalid website URL" })
      .optional()
      .or(z.literal("")),
    acceptTerms: z.boolean().refine((val) => val === true, {
      message: "You must accept the terms and conditions",
...
export const agencyApprovalSchema = z
  .object({
