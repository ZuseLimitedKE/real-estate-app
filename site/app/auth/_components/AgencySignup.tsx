...
import { Building, FileText, User, MapPin, Lock, ChevronLeft, ChevronRight, Calendar as CalendarIcon, Check, Loader2, } from 'lucide-react';
...
const steps = [
  { id: 1, title: 'Company Information', icon: Building, fields: ['companyName', 'tradingName', 'businessType'] },
  { id: 2, title: 'Business Details', icon: FileText, fields: ['registrationNumber', 'licenseNumber', 'taxId', 'establishedDate', 'website', 'description'] },
  { id: 3, title: 'Contact Person', icon: User, fields: ['contactPerson.firstName', 'contactPerson.lastName', 'contactPerson.position', 'contactPerson.email', 'contactPerson.phoneNumber'] },
  { id: 4, title: 'Business Address', icon: MapPin, fields: ['businessAddress.street', 'businessAddress.city', 'businessAddress.state', 'businessAddress.zipCode', 'businessAddress.country'] },
  { id: 5, title: 'Account Setup', icon: Lock, fields: ['email', 'password', 'confirmPassword', 'acceptTerms'] },
];
...
const AgencySignup = () => {
  ...
  // const [prevState, formAction] = useFormState<AuthActionResult | null, FormData>(
  //   // @ts-ignore
  //   registerAgency,
  //   null
  // );
  ...
  const onSubmit = (values: z.infer<typeof agencyRegistrationSchema>) => {
    startTransition(async () => {
     ...
     formData.append('acceptTerms', values.acceptTerms ? 'on' : 'off');
      ...
      if(results.success) {
        toast.success(results.message || 'Registration successful! Please check your email to verify your account.');
        form.reset();
      }
      else {
    ...
    <div className="min-h-screen bg-muted flex items-center justify-center p-4">
      <Card className="w-full max-w-4xl">
        <CardHeader>
          ...
          <CardDescription>Join our platform as a verified real estate agency.</CardDescription>
        </CardHeader>
        <CardContent>
          {/* Stepper */}
              ...
              return (
                <div key={step.id} className="flex-1 flex items-center gap-2">
                  <div className="flex flex-col items-center">
                    ...
                    <p className={cn("text-xs mt-2 text-center", isActive && "text-primary font-semibold")}>
                      {step.title}
                    </p>
                  </div>
                  {index < steps.length - 1 && (
                    <div className={cn("h-0.5 w-full", isCompleted ? "bg-green-600" : "bg-border")} />
                  )}
                </div>
              );
            })}
                ...
                <div className="space-y-6">
                  ...
                  <FormField control={form.control} name="tradingName" render={({ field }) => (
                    <FormItem>
                      ...
                      <FormControl><Input placeholder="e.g. Awesome Properties" {...field} /></FormControl>
                      <FormMessage />
                    </FormItem>
                  )} />
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <FormField control={form.control} name="registrationNumber" render={({ field }) => (
                    <FormItem>
                      ...
                      <FormControl><Input placeholder="C-123456" {...field} /></FormControl>
                      <FormMessage />
                    </FormItem>
                  )} />
                  <FormField control={form.control} name="licenseNumber" render={({ field }) => (
                    <FormItem>
                      ...
                      <FormControl><Input placeholder="RE-98765" {...field} /></FormControl>
                      <FormMessage />
                    </FormItem>
                  )} />
                  <FormField control={form.control} name="taxId" render={({ field }) => (
                    <FormItem>
                      ...
                      <FormControl><Input placeholder="Enter tax identification number" {...field} /></FormControl>
                      <FormMessage />
                    </FormItem>
                  )} />
                  <FormField control={form.control} name="establishedDate" render={({ field }) => (
                  ...
                  <div className="md:col-span-2">
                    <FormField control={form.control} name="description" render={({ field }) => (
                      <FormItem>
                        ...
                        <FormControl><Textarea placeholder="Brief description of your company" className="resize-none" {...field} /></FormControl>
                        <FormMessage />
                      </FormItem>
                    )} />
                  </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <FormField control={form.control} name="contactPerson.position" render={({ field }) => (
                    <FormItem>
                      ...
                      <FormControl><Input placeholder="Managing Director" {...field} /></FormControl>
                      <FormMessage />
                    </FormItem>
                  )} />
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <FormField control={form.control} name="businessAddress.street" render={({ field }) => (
                    <FormItem className="md:col-span-2">
                      ...
                      <FormControl><Input placeholder="123 Business Rd" {...field} /></FormControl>
                      <FormMessage />
                    </FormItem>
                  )} />
                  ...
                  <FormField control={form.control} name="businessAddress.country" render={({ field }) => (
                    <FormItem>
                  ...
                  <FormField control={form.control} name="acceptTerms" render={({ field }) => (
                    <FormItem className="flex flex-row items-start space-x-3 space-y-0 rounded-md border p-4">
                      ...
                      <div className="space-y-1 leading-none">
                        <FormLabel>I accept the terms and conditions and privacy policy *</FormLabel>
                      </div>
                    </FormItem>
                  )} />
                </div>
