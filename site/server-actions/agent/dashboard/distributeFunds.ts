...
export default async function distributeFund(investor: DistributePropertyInvestor, totalAmount: number, decimals: number, tokenAddress: string) {
    try {
        await requireRole("agency");

        // Parse details to send to registerContract
        await realEstateManagerContract.distributeFund(
