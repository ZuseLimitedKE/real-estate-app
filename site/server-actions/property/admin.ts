...
import { agencyApprovalSchema, adminUserCreationSchema, User, UserStatus, AgencyUser  } from '@/types/auth';
import { sendAgencyApprovalEmail, sendAgencyRejectionEmail } from '@/lib/utils/email';
...
export async function searchUsers(
    ...
    try {
        const { db } = await require('@/lib/database/connection').connectToDatabase();
...
export async function getAgencyDetails(agencyId: string): Promise<AgencyUser | null> {
    await checkAdminAccess();
